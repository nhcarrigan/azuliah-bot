    ...
    try {
      ...
      if (
        ...
        await interaction.editReply({
          content: "This command can only be run by administrators.",
        });
        return;
      }
      ...
      if (!("send" in channel)) {
        await interaction.editReply({
          content: "I cannot create roles in a non-text channel.",
        });
        return;
      }
      const roleArray = [
      ...
      await interaction.editReply({
        content: "Your role post has been generated.",
      });
    } catch (err) {
      await errorHandler(bot, "roles command", err);
      await interaction.editReply({
